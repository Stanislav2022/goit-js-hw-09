...
function handleSubmit(evt) {
    evt.preventDefault();
    for(let position1 = 1; position1 <= amount; position1 += 1){
        createPromise(position1, delay).then(({ position , delay: delay1  })=>{
            console.log(`✅ Fulfilled promise ${position} in ${delay1}ms`);
            (0, _notiflixDefault.default).Notify.success(`✅ Fulfilled promise ${position} in ${delay1}ms`);
        }).catch(({ position , delay: delay2  })=>{
            console.log(`❌ Rejected promise ${position} in ${delay2}ms`);
            (0, _notiflixDefault.default).Notify.success(`❌ Rejected promise ${position} in ${delay2}ms`);
        });
        delay += step;
    }
    delay = Number(delayEl.value);
}
function createPromise(position, delay3) {
    return new Promise((resolve, reject)=>{
...
},{"notiflix":"5WWYd","notiflix/build/notiflix-notify-aio":"eXQLZ","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"5WWYd":[function(require,module,exports) {
var global = arguments[3];
